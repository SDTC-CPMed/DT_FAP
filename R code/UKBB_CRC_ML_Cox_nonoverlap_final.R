#============================================================
# UKBB incident CRC survival analysis - non-overlap bin
#============================================================
...
eval_years <- c(2, 4, 6, 8, 10, 12, 14, 16) # bin ENDS: non-overlapping bins are [prev,this], e.g. 0-2, 2-4, ..., 14-16
eval_year_starts <- c(0, 2, 4, 6, 8, 10, 12, 14) # bin STARTS (same length as eval_years)
...
out.dir    <- file.path(out.base, "UKBB_incidentCRC_survival_final_nonoverlap_jtestforgithub")
...
get_model_colors <- function() {
  c(
    ...
    "CEA" = "#5DA5DA",
    "Random" = "grey60",
    "age_sex_bmi_PRS" = "#8E63A9",
    "FDS_age_sex_bmi_PRS" = "#C44A3A"
  )
}
...
analyze_survival_models_incident <- function(
    ...
    eval_year_starts = c(0,2,4,6,8,10,12),
    n_bootstrap = 100,
    alpha = 0,
    ncores = 20,
    exclude_first_n_years = 0,
  ...
  purrr::map_dfr(controls, function(current_control) {
    ...
    foreach::foreach(
      ...
      # tdAUC rows — non-overlapping bins: [bin_start, bin_end]
      # Keep only bins whose END is strictly after exclude_first_n_years
      bin_idx <- which(eval_years > exclude_first_n_years)
      auc_rows <- purrr::map_dfr(bin_idx, function(bi) {
        bin_end   <- eval_years[bi]
        bin_start <- eval_year_starts[bi]
        
        # Adjust both edges relative to the exclusion window
        bin_end_adj   <- bin_end   - exclude_first_n_years
        bin_start_adj <- max(0, bin_start - exclude_first_n_years)
        
        # Restrict test_data to the non-overlapping bin:
        # cases that had event within (bin_start_adj, bin_end_adj]
        # controls that were still event-free at bin_start_adj (time_yrs >= bin_start_adj)
        bin_test <- test_data %>%
          dplyr::filter(
            (event == 1 & time_yrs > bin_start_adj & time_yrs <= bin_end_adj) |
              (event == 0 & time_yrs >= bin_start_adj)
          )
        
        if (nrow(bin_test) < 10 || length(unique(bin_test$event)) < 2) {
          roc_obj <- NULL
        } else {
          bin_lp <- fit_obj$test_lp[match(bin_test$eid, test_data$eid)]
          roc_obj <- tryCatch(
            survivalROC::survivalROC(
              Stime  = bin_test$time_yrs,
              status = bin_test$event,
              marker = bin_lp,
              predict.time = bin_end_adj,
              method = "KM"
            ),
            error = function(e) NULL
          )
        ...
        tibble::tibble(
          ...
          Model = model_name,
          Bootstrap = i,
          Observation_Years = bin_end,      # bin END label (same column name, same structure)
          Bin_Start = bin_start,            # new: bin start for reference
          Control_Group = current_control,
          ExcludeFirstYears = exclude_first_n_years
        )
      })
...
for (exclude_first_n_years in exclude_first_n_years_list) {
  ...
  for (i in seq_along(runs_list)) {
    ...
    res <- analyze_survival_models_incident(
      ...
      eval_year_starts = eval_year_starts,
...
plot_tdauc_models <- function(summary_auc, models, title, outfile_stub) {
  p <- summary_auc %>%
    dplyr::filter(ExcludeFirstYears == 0, Model %in% models) %>%
    ...
    labs(
      ...
      x = "Prediction window end (years; non-overlapping bins)",
...
plot_cindex_models <- function(summary_cindex, models, title, outfile_stub) {
  ...
  ggsave(file.path(out.dir, paste0(outfile_stub, ".pdf")), p, width = 5.8, height = 3.4, dpi = 300)
  ggsave(file.path(out.dir, paste0(outfile_stub, ".png")), p, width = 5.8, height = 3.4, dpi = 300)
}
